        ...
        cy_pdstack_app_resp_cbk_t app_resp_handler)
{
    ...
    for(snk_pdo_index = 0u; snk_pdo_index < snk_pdo_len; snk_pdo_index++)
    {
        ...
        {
#if (CY_PD_EPR_ENABLE)
            cy_stc_pdstack_pd_packet_epr_t *eprSrcCapP = (cy_stc_pdstack_pd_packet_epr_t *)srcCap;
            if((src_pdo_index >= CY_PD_MAX_NO_OF_PDO) && (eprSrcCapP->dat[src_pdo_index].fixed_src.supplyType == CY_PDSTACK_PDO_BATTERY))
            {
                continue;
            }
#endif /* CY_PD_EPR_ENABLE */
